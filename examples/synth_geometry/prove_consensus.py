...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        (models / "W.json").write_text(
            json.dumps(
                make_model.build_matrix(rows=128, cols=32, seed=20260601),
                separators=(",", ":"),
                sort_keys=True,
            )
        )
        ...
        for u in range(1, N_UNITS + 1):
            ...
            canon = {
                json.dumps(
                    {"exit_code": e["exit_code"], "payload": e["payload"]},
                    sort_keys=True,
                    separators=(",", ":"),
                )
                for e in envelopes
            ...
            _check(
                f"{unit.unit_id}: {REPLICAS} replicas byte-identical",
                len(canon) == 1,
            ...
            _check(
                f"{unit.unit_id}: hash_agreement_reducer agreed (N={REPLICAS})",
                outcome.agreed and outcome.agreeing_workers == REPLICAS,
                detail=f"semantic_hash={(outcome.semantic_hash or '')[:16]}…",
            )

            per_unit_metric.append(float(envelopes[0]["payload"]["mean_abs_cosine_separation"]))
        ...
        _check(
            "experiment-level reduce produced a fingerprint", fingerprint["unit_count"] == N_UNITS
        )
